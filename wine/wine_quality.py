...
wine = pd.read_csv('datascience/wine/winequality-both.csv', sep=',', header=0)
...
print(red_wine.plot.hist())
print(white_wine.plot.hist())


# Pairwise Relationships and Correlation: corr between all pairs of variables
...
colMap={"red":"red","white":"white"}
cols=list(map(lambda x:colMap.get(x),wine_sample.type))
# diagonal='kde' or 'hist'
# can't find a way to color by 'type'
pd.plotting.scatter_matrix(
    wine_sample[['quality', 'alcohol', 'residual_sugar']], marker='o'
    , alpha=0.2, figsize=(6, 6), diagonal='hist', c=cols, grid=True
    # , hist_kwds={'color':['burlywood','red']}
)
...
# columns.difference: Return a new Index with elements from the index that are not in parameter.
independent_variables = wine[wine.columns.difference(['quality', 'type', 'in_sample'])]
independent_variables_standardized = (independent_variables - independent_variables.mean()) / independent_variables.std()
wine_standardized = pd.concat([dependent_variable, independent_variables_standardized], axis=1)
lm_standardized = ols(my_formula, data=wine_standardized).fit()
# each coefficient represents the avg std difference in wine quality
